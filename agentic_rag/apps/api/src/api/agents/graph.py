...
from api.agents.agents import coordinator_agent, product_qa_agent, shopping_cart_agent
...
from api.agents.tools import add_to_shopping_cart, remove_from_cart, get_shopping_cart
...
def coordinator_agent_edge(state):
    ...
    if state.coordinator_agent.final_answer and len(state.coordinator_agent.plan) == 0:   # 是否为最终回答
        ...
        return "end"
    elif state.coordinator_agent.next_agent == "product_qa_agent":          # 用户意图为商品信息检索 product_qa_agent
        return "product_qa_agent"
    elif state.coordinator_agent.next_agent == "shopping_cart_agent":       # 用户意图为购物车增删改 shopping_cart_agent
